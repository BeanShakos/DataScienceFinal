...
I also downloaded a csv from the world bank website containing the HDI index and other indicator of the countries

```{r}
library(dplyr)
library(ggplot2)
data <- read.csv("world_bank_indicators.csv")
hdiData <- read.csv("HDI_data.csv")
head(hdiData)


#HDI Data had a lot more cleanup needed at the start
hdiData <- hdiData[,-c(4,6,8,10,12,14,16,18)]
hdiData <-hdiData [, 1:10]
colnames(hdiData)<-hdiData[4,]
hdiData<-hdiData[-c(1:5), ]
hdiData[hdiData == ".."] <- NA
hdiData[hdiData == ""] <- NA
hdiData[,3:10] <- as.data.frame(apply(hdiData[,3:10], 2, as.numeric))
...
```{r}
#First I am removing the indicators that are also related to income inequality since they will clearly have the highest correlation

data <- data %>% select(-Income.share.held.by.highest.10.,-Income.share.held.by.lowest.10.,-Income.share.held.by.lowest.20.)
select_indicators <- function(target_var,num_vars){
...
```{r}
Gini_indicators <- sort(select_indicators("Gini.index",20))
bar_colors <- ifelse(Gini_indicators > 0, "skyblue", "salmon")
print(Gini_indicators)
barplot(
  sort(Gini_indicators),
...
```{r}
Gini_indicators <- sort(select_indicators("Gini.index",6))
bar_colors <- ifelse(Gini_indicators > 0, "skyblue", "salmon")
print(Gini_indicators)
barplot(
  Gini_indicators,
...
# Looking at HDI over time
```{r}
years <- colnames(hdiData)[3:10]
print(Gini_indicators)
barplot(
  colMeans(hdiData[,years], na.rm = TRUE),
  main = "global average of HDI over the years",
  xlab = "Years",
  ylab = "HDI value",
  las = 2,
  cex.names = 0.8,
...
```{r}
par(mfrow = c(1,5))
boxplot(hdiData$"1990", main = "1990", ylim = c(0,1))
boxplot(hdiData$"2000", main = "2000", ylim = c(0,1))
boxplot(hdiData$"2010", main = "2010", ylim = c(0,1))
boxplot(hdiData$"2020", main = "2020", ylim = c(0,1))
boxplot(hdiData$"2023", main = "2023", ylim = c(0,1))
...
Something of note though is that despite how far back the data goes, the ability to measure the data globally and holistically is a fairly new feat. It would take another decade before we have consist data tracking for the entire world.
- For now we can see that Gini coefficient isn't a sufficient indicator of how a country is doing. Gini coefficient has negative correlations with a lot of negative traits like lack of education and high adolescent birth rates. This shows us that lower income countries may be more equitable in income but have lower quality of life overall.
- Overall there seems to be a negative correlation with quality of life and income equity. More research is needed to see if this is a causal relationship but my inference is that the lack of equity is just a side effect of the way these higher income countries developed.
- Other next steps include bolstering efforts to improve widespread data collection and research what factors are the most efficient at eliminating global poverty and increasing quality of life.